...
class caikuSpider(scrapy.Spider):
    name = "caiku"
    ...
    def start_requests(self):
        for index, key_word in enumerate(settings.KEY_WORDS): # enumerate返回'元素下标'和'元素内容'
            ...
            # url = 'http://lp.search.gov.hk/search.html?query=' + kw_quote + '&ui_lang=zh-cn&tpl_id=stdsearch&page=1&p_size=10&output=xml_no_dtd&ui_charset=utf-8&a_submit=false&gp0=swd_home&gp1=swd_home&doc_type=all&web=this&txtonly=0&site=swd_home&last_mod=%23-1&tab=&tab_depts=&sort=date'
            url = 'http://lp.search.gov.hk/search?query=' + kw_quote + '&ui_lang=zh-hk&tpl_id=stdsearch&page=1&p_size=10&output=xml_no_dtd&ui_charset=utf-8&a_submit=false&gp0=fstb_home&gp1=fstb_home&doc_type=all&web=this&txtonly=0&site=fstb_home&last_mod=%23-1&tab=&tab_depts=&sort=date'
    ...
    def parse_content(self, response):
        ...
        try:
            ...
            con3 = soup.find('table', width = '579')
            if con1:
                print '+++++++++++++++++++'
